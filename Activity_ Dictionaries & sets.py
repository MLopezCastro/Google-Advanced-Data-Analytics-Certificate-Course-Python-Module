...
print(epa_tuples[:5])  # Displaying only the first 5 records for readability
...
for state, county, aqi in epa_tuples:
    # If the state is not in the dictionary, initialize it with an empty list
...
# Print an example lookup
print(aqi_dict.get("Vermont", "State not found"))  # Example lookup for Vermont
...
# Retrieve the list of (county, AQI) tuples for Arizona
...
# Retrieve the list of (county, AQI) tuples for California
california_readings = aqi_dict.get("California", [])

# Extract only the AQI values from the tuples
california_aqi_values = [aqi for _, aqi in california_readings]

# Calculate the mean AQI
mean_california_aqi = sum(california_aqi_values) / len(california_aqi_values) if california_aqi_values else 0
...
def county_counter(state):
    """
    Takes a state name as input and returns a dictionary with counties as keys
    and the count of occurrences in AQI data as values.
    """
    # Retrieve county-AQI tuples for the given state
    state_data = aqi_dict.get(state, [])

    # Initialize an empty dictionary to store county counts
    county_dict = {}

    # Loop through state data and count occurrences of each county
    for county, _ in state_data:
        county_dict[county] = county_dict.get(county, 0) + 1

    return county_dict

# Example usage
print(county_counter("Florida"))  # Example call for Florida
...
# Call the county_counter function for Pennsylvania
pa_counties = county_counter("Pennsylvania")

# Retrieve the count for Washington County
...
# Call the county_counter function for Indiana
indiana_counties = county_counter("Indiana")

# Get the dictionary keys (county names)
county_names = indiana_counties.keys()
...
# Initialize an empty list to store all counties
all_counties = []

# Loop through each state in aqi_dict
for state in aqi_dict:
    # Get all county names for the state using county_counter
    state_counties = county_counter(state).keys()
    
    # Extend all_counties list with the county names
    all_counties.extend(state_counties)

# Print the length of all_counties
...
# Convert all_counties to a set to remove duplicates
unique_counties = set(all_counties)

# Calculate the number of duplicate county names
shared_counties_count = len(all_counties) - len(unique_counties)

# Print the result
print(shared_counties_count)
